...
public class ImportData(IDbContextFactory<InventoryContext> contextFactory, ILogger<ImportData> logger, ImageStorageService imageStorage, RebrickableApi rebrickable)
{
    ...
    public async Task BackfillImagesAsync(IProgress<(int done, int total)>? progress = null, CancellationToken ct = default)
    {
        ...
        var minifigs = await context.Set<Minifig>().Where(m => m.ImgUrl != null && m.ImgUrl.StartsWith("http")).ToListAsync(ct);
        ...
        var minifigWork = minifigs.Select((m, i) => (i, src: m.ImgUrl!, key: $"minifigs/{m.MinifigId}.jpg")).ToArray();
        ...
        for (int i = 0; i < minifigs.Count; i++) if (minifigUrls[i] != null) minifigs[i].ImgUrl = minifigUrls[i];
