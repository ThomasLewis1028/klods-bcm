...
public static class HomeEndpoints
{
    public static void MapHome(this IEndpointRouteBuilder app)
    {
        app.MapGet("/api/home/preview", async (IDbContextFactory<InventoryContext> dbFactory) =>
        {
            ...
            var minifigs = await db.Set<Minifig>().AsNoTracking().Where(m => m.MinifigImgUrl != null).ToListAsync();
            ...
            return Results.Ok(new HomePreviewDto(
                ...
                PickDistinct(minifigs, 2).Select(m => new PreviewItemDto(m.MinifigId, m.MinifigName, m.MinifigImgUrl)).ToList()
            ));
